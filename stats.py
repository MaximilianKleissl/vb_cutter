...
class Stats:
    ...
    def validate(self):
        ...
        assert "b" not in rally_markers[1::2]
        assert "B" not in rally_markers[1::2]
        assert "w" not in rally_markers[::2]
        assert "W" not in rally_markers[::2]
    ...
    def get_stats(self):
        my_stats = {
            "Points won": len([1 for m, t in self.markers if m == "w"]),
            "Points loose": len([1 for m, t in self.markers if m == "W"]),
            "Attacks over 1": len([1 for m, t in self.markers if m == "1"]),
            "Points Won/Lost after Attack over 1": self.count_followed_by_win_and_lose(
                "1"
            ),
        }
