...
class Stats:
  ...
  def get_stats(self):
    ...
    my_stats["Points Won/Lost after Attack over 1"] = self.count_followed_by_win_and_lose("1")
    my_stats["Attacks over 2"] = len([1 for m, t in self.markers if m == "2"])
    my_stats["Points Won/Lost after Attack over 2"] = self.count_followed_by_win_and_lose("2")
    my_stats["Attacks over 3"] = len([1 for m, t in self.markers if m == "3"])
    my_stats["Points Won/Lost after Attack over 3"] = self.count_followed_by_win_and_lose("3")
    my_stats["Attacks over 4"] = len([1 for m, t in self.markers if m == "4"])
    my_stats["Points Won/Lost after Attack over 4"] = self.count_followed_by_win_and_lose("4")
    my_stats["Attacks over 5"] = len([1 for m, t in self.markers if m == "5"])
    my_stats["Points Won/Lost after Attack over 5"] = self.count_followed_by_win_and_lose("5")
    my_stats["Attacks over 6"] = len([1 for m, t in self.markers if m == "6"])
    my_stats["Points Won/Lost after Attack over 6"] = self.count_followed_by_win_and_lose("6")
    return my_stats
