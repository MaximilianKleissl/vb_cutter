    ...
    for idx, row in df.iterrows():
        ...
        if row["Last Set By Us"]:
            ...
            if row["Marker"] == "6":
                ...
                continue
            if row["Marker"] == "2":
                players.append(row["Our Lineup"][{1: (4-1 if row["Last Serve By Us"] else 2-1), 2: 4-1, 3: 2-1, 4: 3-1, 5: 2-1, 6: 3-1}[row["Our Rotation"]]])
                continue
            if row["Marker"] == "3":
                players.append(row["Our Lineup"][{1: 3-1, 2: 4-1, 3: 2-1, 4: 3-1, 5: 4-1, 6: 2-1}[row["Our Rotation"]]])
                continue
            if row["Marker"] == "4":
                players.append(row["Our Lineup"][{1: (2-1 if row["Last Serve By Us"] else 4-1), 2: 3-1, 3: 4-1, 4: 2-1, 5: 3-1, 6: 4-1}[row["Our Rotation"]]])
                continue
        else:
            if row["Marker"] == "1":
            ...
            if row["Marker"] == "2":
                ...
                continue
            if row["Marker"] == "3":
                players.append(row["Opp Lineup"][{1: 3, 2: 6-1, 3: 2-1, 4: 3-1, 5: 4-1, 6: 2-1}[row["Opponent Rotation"]]])
                continue
            if row["Marker"] == "4":
                players.append(row["Opp Lineup"][{1: (4-1 if row["Last Serve By Us"] else 2-1), 2: 3-1, 3: 2-1, 4: 2-1, 5: 3-1, 6: 4-1}[row["Opponent Rotation"]]])
                continue
        if row["Marker"] == "b":
...
def get_dict(markers):
    ...
    for marker in df['Marker']:
        ...
        if marker.startswith("P"):
            opp_lineup[opp_lineup.index(int(marker[1:].split(":")[0]))] = int(marker[1:].split(":")[1])
        if marker == "b":
            rally_begin_with_our_serve = True
        if marker == "B":
