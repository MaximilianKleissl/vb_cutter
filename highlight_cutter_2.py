...
class CutterEditor:
    ...
    def create_widgets(self):
        ...
        self.filter_column_1 = ttk.Combobox(self.master, values=list(self.marker_df.columns))
        ...
        self.filter_column_2 = ttk.Combobox(self.master, values=list(self.marker_df.columns))
        ...
        self.filter_column_3 = ttk.Combobox(self.master, values=list(self.marker_df.columns))
    ...
    def load(self):
        ...
        if column_1 != "Select Column":
            self.filtered_df = self.marker_df[self.marker_df[column_1].astype(str) == value_1]
        else:
            self.filtered_df = self.marker_df
        if column_2 != "Select Column":
            self.filtered_df = self.filtered_df[self.filtered_df[column_2].astype(str) == value_2]
        if column_3 != "Select Column":
            self.filtered_df = self.filtered_df[self.filtered_df[column_3].astype(str) == value_3]

        for i, row in self.filtered_df.iterrows():
            self.tree.insert('', 'end', iid=str(i), values=(row["Marker"], row["Timestamp"], 3, 3))
